 ...
 * content based on the [com.das.downloader.data.model.download.DownloadState] of active tasks.
 *
 * Example usage:
 * ```kotlin
...
class DownloadNotifier(
    ...
    fun notifyState(state: DownloadState) {
        ...
        manager.notify(state.id.toNotificationId(), builder.build())
    }
    ...
    fun cancelNotification(id: String) {
        manager.cancel(id.toNotificationId())
    }

    /**
     * Removes all download notifications from the status bar.
     */
    ...
    /**
     * Converts this string into a stable notification ID.
     *
     * Uses [hashCode] so the same string always produces the same integer.
     */
    private fun String.toNotificationId(): Int = hashCode()
