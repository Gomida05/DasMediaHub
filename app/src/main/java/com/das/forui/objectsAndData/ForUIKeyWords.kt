package com.das.forui.objectsAndData

object ForUIKeyWords {
    const val ACTION_START = "com.das.forui.START_BACKGROUND_MEDIA"
    const val ACTION_KILL = "com.das.forui.kill"
    const val ACTION_ADD_TO_WATCH_LATER = "com.das.forui.ACTION_ADD_TO_WATCH_LATER"
    const val PLAY_HERE_VIDEO = "com.das.forui.PLAY_HERE_VIDEO"

    const val SET_SHUFFLE_MODE = "com.das.forui.SET_SHUFFLE_MODE"

    const val NEW_INTENT_FOR_VIEWER = "com.das.forui.NEW_INTENT_FOR_VIEWER"
    const val NEW_INTENT_FOR_SEARCHER = "com.das.forui.NEW_INTENT_FOR_SEARCHER"
    const val NEW_TEXT_FOR_RESULT = "com.das.forui.NEW_TEXT_FOR_RESULT"


    const val DOWNLOADER_NOTIFICATION_CHANNEL = "com.das.forui.DOWNLOADER_NOTIFICATION_CHANNEL"
    const val AUDIO_SERVICE_FROM_URL_NOTIFICATION = "com.das.forui.services.AudioServiceFromUrl.NOTIFICATION_DELETED"
    const val BACKGROUND_GROUND_PLAYER_NOTIFICATION = "com.das.forui.services.BackgroundPlayer.NOTIFICATION_DELETED"



    const val YOUTUBE_REGEX = "(?<=v=|/)([a-zA-Z0-9_-]{11})(?=&|\$|/)"
    const val YOUTUBE_HOST_1 = "www.youtube.com"
    const val YOUTUBE_HOST_2 = "youtube.com"
    const val YOUTUBE_HOST_3 = "youtu.be"

    const val EXCEPTED_DOWNLOAD_ID = "com.das.forui.DownloaderClass.EXCEPTED_DOWNLOAD_ID"
}